...
(function (dice) {
  ...
  function prepare_rnd(callback) {
    if (!random_storage.length && teal.dice.use_true_random) {
      try {
        teal.rpc({ method: "random", n: 512 }, function (random_responce) {
  ...
  this.create_d4 = function (color) {
    ...
    if (true) //!this.dice_material)
      ...
      this.label_color = teal.getLuminance(this.dice_color) < 128 ? '#cccccc' : '#222222'
  ...
  this.dice_box.prototype.bind_mouse = function (
    ...
    var box = this;
    teal.bind(container, ["mousedown", "touchstart"], function (ev) {
      ev.preventDefault();
      box.mouse_time = new Date().getTime();
      box.mouse_start = teal.get_mouse_coords(ev);
    });
  ...
  this.dice_box.prototype.reroll = function (
    notation_getter,
    before_roll,
    after_roll,
    values) {
    
    var before_reroll = function(vectors, notation, callback) {
      info_div.style.display = 'none';
      selector_div.style.display = 'none';
      callback(values);
    }

    this.start_throw(notation_getter, before_reroll, after_roll);
  }
